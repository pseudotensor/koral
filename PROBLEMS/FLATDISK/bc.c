...
coco_N(xxvec,xxvecBL,MYCOORDS,SPHCOORDS);
xx=xxvec[1];
yy=xxvec[2];
zz=xxvec[3];
...
fill_geometry_arb(ix,iy,iz,&geomBL,SPHCOORDS);
...
calc_g_arb(xxvecBL,ggBL,SPHCOORDS);
calc_G_arb(xxvecBL,GGBL,SPHCOORDS);
ldouble eupBL[4][4],eloBL[4][4];
ldouble tupBL[4][4],tloBL[4][4];
calc_tetrades(ggBL,tupBL,tloBL,SPHCOORDS);
calc_ZAMOes(ggBL,eupBL,eloBL,SPHCOORDS);
/**********************/
...
if(ix>=NX) //analytical solution at rout only
  {
 ...
 else if(ix<0) //outflow near BH
   {
     ...
     if(MYCOORDS==SPHCOORDS || MYCOORDS==SPHCOORDS)
       pp[7]=-1.;
